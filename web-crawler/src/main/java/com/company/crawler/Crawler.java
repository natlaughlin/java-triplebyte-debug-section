...
public class Crawler {
	...
	private boolean uriShouldBeCrawledAsNode(URI uri) {
		...
		// HTTPS <-> HTTP should be crawled
		String domainScheme = domain.getScheme();
		if(domainScheme.endsWith("s")){
			domainScheme = domainScheme.substring(0, domainScheme.length() - 1);
		}
		String uriScheme = uri.getScheme();
		if(uriScheme.endsWith("s")){
			uriScheme = uriScheme.substring(0, uriScheme.length() - 1);
		}
		if (!domainScheme.equals(uriScheme)) {
			return false;
		}
